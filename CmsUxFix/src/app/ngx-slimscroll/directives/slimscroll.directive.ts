...
export class SlimScrollDirective implements OnInit, OnDestroy {
  ...
  getBarHeight(): void {
    const barHeight = Math.max(isNaN((this.el.offsetHeight / this.el.scrollHeight) * this.el.offsetHeight) ? 0 : (this.el.offsetHeight / this.el.scrollHeight) * this.el.offsetHeight, 30) + 'px';
